...
export async function getSubscription(): Promise<SubscriptionInfo> {
  ...
  const subscription = await store.get('subscription') as SubscriptionInfo | undefined;
...
export async function saveSubscription(subscription: SubscriptionInfo): Promise<void> {
  const store = await getStore();
  await store.set('subscription', subscription);
  await store.save();
}
...
export async function getTodayRequestCount(): Promise<number> {
  const store = await getStore();
  const requestCount = await store.get('request_count') as Record<string, number> | undefined;
...
export async function getConvertType(): Promise<string> {
  const store = await getStore();
  return (await store.get('convert_type') as string) || 'revision';
}
...
export async function saveConvertType(convertType: string): Promise<void> {
  const store = await getStore();
  await store.set('convert_type', convertType);
  await store.save();
}
...
export async function getScreenType(): Promise<string | undefined> {
  const store = await getStore();
  return await store.get('screen_type') as string | undefined;
}
